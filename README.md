...
* **个性化推荐:** 基于用户的个人情况（`目标分数`, `目标院校等级`, `偏好计算机等级`, `目标地区(省份或A/B区)`）和加权评分算法，推荐最合适的 Top N 院校 (默认为10所，分页显示)。推荐算法如下：
  ...
  * **权重规则:**
        ...
        | 指标                 | 权重 | 评分规则                                                                 |
        | -------------------- | ---- | ------------------------------------------------------------------------ |
        | 目标分数相似度       | 40%  | 取所有专业2024年（如无则2023年）复试分数线字符串（如330/55/80），提取最大值（如330），与目标分数做差，分数相似度=100-abs(目标分数-最大分数)，小于0则为0。 |
        | 院校等级匹配度       | 20%  | 985=60，211=40，双一流=20，普通院校=0，完全匹配则为100，否则按上述分数。 |
        | 计算机等级匹配度     | 20%  | A+=100，A=80，A-=70，B+=60，B=50，B-=40，C+=30，C=20，C-=10，无等级=0，完全匹配则为100，否则按上述分数。 |
        | 地区匹配 (省份/考区) | 20%  | 省份或region与目标地区完全一致为100，否则为0。 |
