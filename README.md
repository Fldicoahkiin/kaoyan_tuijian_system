...
* **可视化大面板:** 动态展示全国计算机考研相关的宏观数据。主要包括：
  * **中间区域**: 可滚动的院校列表，展示 `院校名称`, `院校等级`, `计算机等级`, `24年招生人数`, `初试科目`, `地区(A/B区)`。
  * **左侧面板**:
...
* **院校库查询:** 允许用户根据 `地区`, `院校等级`, `院校名称`, `计算机等级` 等维度筛选院校，并按 `收藏人数` 排序。查询结果以列表形式展示 `院校名称`, `院校等级`, `计算机等级`, `24年招生人数`, `收藏人数`。
* **院校详情:** 展示院校的详细信息，包括 `学校简介`, `招生院系架构`, `专业代码对照表`, `考试科目`, `招生人数`, `学费学制`。特别地，对于已爬取数据的四川院校，会展示近三年的**复试线趋势图**。
* **个性化推荐:** 基于用户的个人情况（`目标分数`, `目标院校等级`, `目标院校计算机等级`, `目标地区`）和加权评分算法，推荐最合适的 Top 20 院校。推荐算法如下：
  * **推荐评分** = \(0.4 \times \text{分数相似度} + 0.2 \times \text{院校等级分} + 0.2 \times \text{计算机等级分} + 0.2 \times \text{地区匹配分}\)
  * **权重规则:**

        | 指标           | 权重 | 评分规则                                   |
        | -------------- | ---- | ------------------------------------------ |
        | 目标分数       | 40%  | 基于用户预期分数与院校历年分数线的误差计算     |
        | 院校等级       | 20%  | 985=60, 211=40, 双一流=20, 一般=0        |
        | 计算机等级     | 20%  | A+=100, A=80, A-=70, B+=60, ..., C-=10, 无=0 |
        | 地区匹配 (省份) | 20%  | 完全匹配=100, 不匹配=0                     |

  * 输出结果列表展示 `院校等级`, `计算机等级`, `24年招生人数`, `收藏人数`。
* **交互功能**:
...
* **核心展示**: 可视化大面板（基本实现`甲方原话`中描述的各类图表、院校滚动列表）、院校库（查询、排序、分页）、院校详情页（已实现四川院校近三年分数线图表）。
* **用户系统**: 注册、登录、登出、个人资料查看与修改、院校收藏与取消收藏 (含心形图标切换)。
* **推荐系统**: 基于用户偏好和加权算法（具体公式见上文）的 Top 20 院校推荐。
* **管理后台**: 管理员登录、仪表盘、用户管理（列表、创建、删除、详情、切换管理员）、公告管理（列表、添加、删除）、管理员密码修改、院校数据列表查看与**顶层信息编辑**。
* **日志记录**: 关键操作记录到 `logs/app.log` 文件。
* **数据爬取 (部分)**: 已实现针对 **四川大学** 和 **电子科技大学** 的专业目录和近三年分数线的爬取与合并逻辑 (`utils/scraper.py`)。
* **Bug 修复**: 解决了后台用户管理表单中 `BooleanField` 未定义的错误。
...
* **[待完成]** **前端页面 (`templates/`, `static/`)**:
  * **[已开始]** UI 界面美化，应用 Bootstrap 5。
  * **[进行中]** 实现深色仪表盘风格主题 (`static/css/dark_theme.css`)。
  * **[待完成]** 可视化大面板: 实现 ECharts 图表 (国家线、比例图等)。
* **[待完成]** **后台功能**:
...
3. **准备数据**: 确保 `data/schools.json`, `data/national_lines.json`, `data/announcements.json` 文件存在且格式正确。如果需要测试管理员功能，请先注册一个用户，然后手动修改其 `data/users/用户名.json` 文件，添加 `"is_admin": true`。
4. **运行应用**: 在项目根目录下，激活虚拟环境后运行：

    ```bash
    python app.py
    ```

    应用将在本地 `http://127.0.0.1:5000/` 上运行。

## 10. 注意事项与安全提示

* **数据存储**: 本项目使用 **JSON 文件** 存储所有核心数据 (院校、用户、公告等)。
  * **优点**: 部署简单，无需额外数据库服务。
  * **缺点**:
    * **性能**: 对于大量数据或高并发读写，文件 I/O 会成为严重瓶颈。
    * **并发控制**: 同时写入文件可能导致数据损坏或丢失，本项目仅做了基础的文件读写，没有实现复杂的锁机制。
    * **数据一致性**: 难以保证复杂操作的原子性。
    * **查询效率**: 复杂查询（如多条件过滤、连接）效率低下，需要加载整个文件到内存。
  * **强烈建议**: 对于任何实际生产用途或数据量增长的应用，**应迁移到关系型数据库 (如 PostgreSQL, MySQL) 或适合的 NoSQL 数据库**。
* **用户认证与安全**:
  * 虽然使用了 `werkzeug` 对密码进行哈希处理，比明文存储安全，但将用户数据（包括密码哈希）存储在可被直接访问的 JSON 文件中**存在显著安全风险**。如果服务器文件系统被攻破，用户数据可能泄露。
  * **生产环境必须使用更安全的认证机制和数据存储方案**。
* **数据备份**: 由于数据存储在 `data/` 目录，**必须定期备份该目录**，以防数据丢失或损坏。
* **爬虫礼仪**: `utils/scraper.py` 中包含了 `time.sleep()` 来避免过于频繁地请求目标网站。在运行爬虫时，请遵守目标网站的 `robots.txt` 规则（如果存在）并保持合理的请求频率。
* **错误处理与日志**: 应用包含基本的日志记录 (`logs/app.log`)。在生产环境中，可能需要配置更详细的日志级别、日志轮转以及集中的错误监控系统。
